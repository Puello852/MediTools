...
export class PedirCitaPage implements OnInit {
  ...
  medico: any;
  tipoAtencions: any;
  constructor(public toastController: ToastController,public alertController: AlertController,public loadingController: LoadingController, private api: ApiToolsService, navParams: NavParams, public modalController: ModalController) {
  ...
  async next(e) {
    this.mesNumber = moment().add(1,'month').format('MM')
    const loading = await this.loadingController.create({
      message: 'Por favor espere...',
    });
    await loading.present();
    ...
    if(this.tipoAtencions == 1){
     ...
     swiper.slideNext();
      ...
      console.log("otro tipo de atencion")
    }
